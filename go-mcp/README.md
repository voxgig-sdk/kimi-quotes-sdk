...
[MCP](https://modelcontextprotocol.io) server exposing the KimiQuotes SDK as
two agent tools — `kimi-quotes_list` and `kimi-quotes_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kimi-quotes-mcp)
make build

# 2. Provide credentials via the environment
export KIMI_QUOTES_APIKEY=sk_live_xxx
...
```jsonc
// kimi-quotes_list: first page of records
{ "entity": "quote" }
{ "entity": "quote", "query": { } }

// kimi-quotes_load: one record by id
{ "entity": "quote", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kimi-quotes-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user kimi-quotes \
     -- "$PWD"/dist/*/kimi-quotes-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `kimi-quotes_list` and `kimi-quotes_load` tools now appear
   in new sessions. Ask the agent to *"list quote using kimi-quotes"*
   and it calls `kimi-quotes_list` with `{"entity":"quote"}`.
...
```sh
export KIMI_QUOTES_APIKEY=sk_live_xxx            # API key
export KIMI_QUOTES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `kimi-quotes_list` tool
...
### Call the `kimi-quotes_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `kimi-quotes_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `kimi-quotes_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `KIMI_QUOTES_APIKEY` | API key sent with every request. |
| `KIMI_QUOTES_BASE` | Optional override of the API base URL. |
