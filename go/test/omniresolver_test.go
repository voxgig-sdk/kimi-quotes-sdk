...
import (
	...
	omni "github.com/voxgig-sdk/kimi-quotes-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.KimiQuotesError); is {
				out := map[string]any{
					"name":    "KimiQuotesError",
...
func omniCtx(arg any, client *sdk.KimiQuotesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.KimiQuotesSDK); is {
			client = live
