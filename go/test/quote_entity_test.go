...
func TestQuoteEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set KIMIQUOTES_TEST_QUOTE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func quoteBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("KIMIQUOTES_TEST_QUOTE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["KIMIQUOTES_TEST_QUOTE_ENTID"])
	...
	if env["KIMIQUOTES_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewKimiQuotesSDK(core.ToMapAny(mergedOpts))
	}
