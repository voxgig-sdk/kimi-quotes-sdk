...
//
// NewKimiQuotesSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.KimiQuotesSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewKimiQuotesSDK(opts)
}
...
func fcCandidates(client *sdk.KimiQuotesSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.KimiQuotesSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.KimiQuotesSDK, name string) any {
