...
//     (\`new KimiQuotesSDK(...)\` / \`KimiQuotesSDK.test(...)\` become
//     \`KimiQuotesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"quote":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new KimiQuotesSDK(...)\` and any
// \`KimiQuotesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+KimiQuotesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/KimiQuotesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
