...
boru-driven command-line client **and** interactive REPL for the KimiQuotes
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kimi-quotes-cli)
make build

# 2. See usage (words, entities, env vars)
./kimi-quotes-cli --help

# 3. Provide credentials once, via the environment
export KIMI_QUOTES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./kimi-quotes-cli list quote
./kimi-quotes-cli load 1 quote            # {id:1} shorthand
./kimi-quotes-cli load '{id:1}' quote       # explicit match map

# 5. Override the API base URL for a single call
KIMI_QUOTES_BASE=https://api.example.com ./kimi-quotes-cli list quote

# 6. No arguments -> interactive REPL
./kimi-quotes-cli
kimi-quotes> list quote
kimi-quotes> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kimi-quotes-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/kimi-quotes-cli list quote
   ```
...
```sh
./kimi-quotes-cli load 1 quote          # scalar shorthand for {id:1}
./kimi-quotes-cli load '{id:1}' quote     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `kimi-quotes>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `quote`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/kimi-quotes-cli`. |
...
(`./kimi-quotes-cli <expr>`) and interactively (the REPL), and expressions compose the
