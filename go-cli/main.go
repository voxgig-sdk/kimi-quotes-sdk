// Command KimiQuotes-cli is a CLI + REPL for the KimiQuotes SDK,
...
const prompt = "kimi-quotes"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "quote"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      KIMI_QUOTES_APIKEY (api key), KIMI_QUOTES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: KIMI_QUOTES_APIKEY carries the API key and
	// KIMI_QUOTES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("KIMI_QUOTES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("KIMI_QUOTES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewKimiQuotesSDK(opts)
