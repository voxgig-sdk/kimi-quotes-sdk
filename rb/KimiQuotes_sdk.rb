...
require_relative 'KimiQuotes_types'
...
class KimiQuotesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = KimiQuotesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = KimiQuotesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, KimiQuotesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    KimiQuotesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = KimiQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = KimiQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = KimiQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => KimiQuotesError.new(
        "#{op}_allow",
        "KimiQuotesSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = KimiQuotesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = KimiQuotesError.new(
        "graphql_error", "KimiQuotesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
