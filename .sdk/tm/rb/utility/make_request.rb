...
module KimiQuotesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = KimiQuotesResponse.new({})
    ...
    if fetch_err
      ...
      response = KimiQuotesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = KimiQuotesResponse.new(fetched)
    else
