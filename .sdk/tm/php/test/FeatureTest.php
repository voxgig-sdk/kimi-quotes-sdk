...
class FtCtrl extends KimiQuotesControl
{
...
class FtClient
{
    ...
    private KimiQuotesUtility $utility;

    public function __construct(KimiQuotesUtility $utility, array $options)
    {
    ...
    public function get_utility(): KimiQuotesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KimiQuotesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public KimiQuotesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KimiQuotesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KimiQuotesConfig::make_feature($name);
    ...
    private static function buildUrl(KimiQuotesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KimiQuotesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KimiQuotesResponse($fetched);
        }
    ...
    private function fail(KimiQuotesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KimiQuotesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
