...
class KimiQuotesTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            KimiQuotesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = KimiQuotesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = KimiQuotesTestRunner.getenv("KIMI_QUOTES_TEST_LIVE")
        override = KimiQuotesTestRunner.getenv("KIMI_QUOTES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = KimiQuotesTestRunner.getenv(key)
        ...
        explain = KimiQuotesTestRunner.getenv("KIMI_QUOTES_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if KimiQuotesTestRunner._test_control is not None:
            return KimiQuotesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                KimiQuotesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            KimiQuotesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = KimiQuotesTestRunner.load_test_control()
...
def load_env_local():
    KimiQuotesTestRunner.load_env_local()


def env_override(m):
    return KimiQuotesTestRunner.env_override(m)


def entity_data(v):
    return KimiQuotesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return KimiQuotesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return KimiQuotesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return KimiQuotesTestRunner.load_test_control()


def live_client_options():
    return KimiQuotesTestRunner.live_client_options()


def live_delay_ms():
    return KimiQuotesTestRunner.live_delay_ms()
